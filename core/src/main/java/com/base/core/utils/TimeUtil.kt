...
object TimeUtil {
    ...
    @Throws(ParseException::class)
    fun dateDiff2(date: String, format: String = "dd MMMM yyyy"): String {
    ...
    @Throws(ParseException::class)
    fun dateDiff4(date: String, format: String = "MMMM dd yyyy"): String {
        val inputFormatString = "yyyy-MM-dd"
        val inputDateFormat = SimpleDateFormat(inputFormatString, Locale.getDefault())
        val inputDate = inputDateFormat.parse(date)
        val outputFormat = SimpleDateFormat(format, Locale.getDefault())
        return outputFormat.format(inputDate)
